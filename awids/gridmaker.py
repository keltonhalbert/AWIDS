...
class Gridmaker( Projection ):
  ...
  def grid( self, **kwargs ):
    ...
    ## a dictionary of expected plot types, their color maps, and the range of values to be plotted
  ...
  def grid_3hr( self, **kwargs ):
    ...
    mycm=mpl.colors.LinearSegmentedColormap.from_list('mycm',c)
    plotparms = { '3TPF': ('3 Hour Temperature (F) Tendency', np.arange(-20,20,1), plt.cm.RdBu_r, 'TMPF'), '3TPC': ('3 Hour Temperature (C) Tendency', np.arange(-15,16,1), plt.cm.RdBu_r, 'TMPC'), '3DWF': ('Three Hour Dewpoint (F) Tendency', np.arange(-20,20,1), plt.cm.RdBu_r, 'DWPF'), '3DWC': ('3 Hour Dewpoint (C) Tendency', np.arange(-15,16,1), plt.cm.RdBu_r, 'DWPC'), '3PRS': ('3 Hour Sea Level Pressure (mb) Tendency', np.arange(-8,8.5,.5),plt.cm.RdBu_r, 'PRES'), '3THA': ('3 Hour Theta (K) Tendency', np.arange(-30,31,1), plt.cm.RdBu_r, 'THTA'), '3THE': ('3 Hour Theta-E (K) Tendency', np.arange(-30,31,1), plt.cm.RdBu_r, 'THTE'), '3VOR': (r'3 Hour Vorticity ($S^{-1}*{10^5}$) Tendency', np.arange(-20,21,1), mycm, 'VORT'), '3DIV': (r'3 Hour Surface Divergence ($S^{-1}*{10^5}$) Tendency', np.arange(-20,21,1), mycm, 'DIVR') }
    name = plotparms[ DataType ][0]
    levs = plotparms[ DataType ][1]
    cmap = plotparms[ DataType ][2]
  ...
  def TriangleMethod( self, **kwargs ):
    DataType = kwargs.get( 'datatype' )
    DataDict = kwargs.get( 'datdict' )
  ...
  def AdvectionGrid( self, **kwargs ):
    ...
    if DataType.upper() == 'TPFA':
      ...
      mycm=mpl.colors.LinearSegmentedColormap.from_list('mycm',c)
      cmap = mycm
      tf_grid = self.grid( datatype='TMPF', datdict=DataDict )
    ...
    if DataType.upper() == 'TPCA':
      ...
      mycm=mpl.colors.LinearSegmentedColormap.from_list('mycm',c)
      cmap = mycm
      tc_grid = self.grid( datatype='TMPC', datdict=DataDict )
    ...
    if DataType.upper() == 'MXRA':
      ...
      mycm=mpl.colors.LinearSegmentedColormap.from_list('mycm',c)
      cmap = mycm
      mx_grid = self.grid( datatype='MIXR', datdict=DataDict )
    ...
    if DataType.upper() == 'THEA':
      ...
      mycm=mpl.colors.LinearSegmentedColormap.from_list('mycm',c)
      cmap = mycm
      the_grid = self.grid( datatype='THTE', datdict=DataDict )
